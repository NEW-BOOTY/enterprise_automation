...
echo "[INFO] Starting setup for ibm at $(gdate -Iseconds)"
install_homebrew_mac
ensure_tools git jq yq gnu-sed coreutils openjdk node ansible helm kubectl
...
echo "COMPANY=ibm" > "$ROOT_DIR/generated/env.meta"
echo "TOOLS=git jq yq gnu-sed coreutils openjdk node ansible helm kubectl" >> "$ROOT_DIR/generated/env.meta"

echo "[INFO] Completed bootstrap for ibm."
