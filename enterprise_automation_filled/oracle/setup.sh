...
echo "[INFO] Starting setup for oracle at $(gdate -Iseconds)"
install_homebrew_mac
ensure_tools git jq yq gnu-sed coreutils maven openjdk kubectl helm docker terraform ansible
...
echo "COMPANY=oracle" > "$ROOT_DIR/generated/env.meta"
echo "TOOLS=git jq yq gnu-sed coreutils maven openjdk kubectl helm docker terraform ansible" >> "$ROOT_DIR/generated/env.meta"

echo "[INFO] Completed bootstrap for oracle."
