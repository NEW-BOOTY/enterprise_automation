...
echo "[INFO] Starting setup for openai at $(gdate -Iseconds)"
install_homebrew_mac
ensure_tools git jq yq gnu-sed coreutils python@3.11 node docker kubectl
...
echo "COMPANY=openai" > "$ROOT_DIR/generated/env.meta"
echo "TOOLS=git jq yq gnu-sed coreutils python@3.11 node docker kubectl" >> "$ROOT_DIR/generated/env.meta"

echo "[INFO] Completed bootstrap for openai."
